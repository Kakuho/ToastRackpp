...
class BaseLoader{
  ...
  void PrintRawBytes(std::size_t n) const{
    bool midbreak = false;
    std::cout << std::setw(8) << std::setfill('0') << 0 << std::setw(3) << std::setfill(' ') << ' ';
    for(std::size_t i = 1; i < n + 1; i++){
      std::cout << std::hex << std::setw(3) << std::setfill('0') << std::bitset<8>(bytes[i - 1]).to_ulong();
